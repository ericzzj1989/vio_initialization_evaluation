...
def analyze_multiple_trials(results_dir, est_type, n_trials,
    ...
    for trial_i in range(n_trials):
        ...
        if traj.data_loaded:
            ...
            traj.compute_initilization_quality()
        if traj.success:
...
if __name__ == '__main__':
    ...
    for est_type_i, plot_dir_i in zip(args.est_types, plots_dirs):
        ...
        fig.savefig(plot_dir_i+'/scale_error'+'_'+plot_traj.align_str+FORMAT,
                    bbox_inches='tight')
        # plot trajectory scale top view and side view
        ...
        labels = ['Estimate']
        colors = ['b']
        # plot relative translation error
        ...
        fig.savefig(plot_dir_i+'/rel_translation_error' + suffix + FORMAT,
                    bbox_inches="tight")
        plt.close(fig)
        # plot relative translation error percentage
        fig = plt.figure(figsize=(6, 2.5))
        ax = fig.add_subplot(
            111, xlabel='Distance traveled [m]',
        ...
        fig.savefig(plot_dir_i+'/rel_yaw_error' + suffix + FORMAT,
                    bbox_inches="tight")
        plt.close(fig)
        # plot initialization quality
        fig = plt.figure(figsize=(8, 2.5))
        ax = fig.add_subplot(
            111, xlabel='Frame id [number]', ylabel='Accumulate Scale. [None]',
            xlim=[0, len(plot_traj.cum_scales)])
        pu.plot_error_n_dim(
            ax, range(len(plot_traj.cum_scales)),
            np.reshape(plot_traj.cum_scales, (-1, 1)),
            plot_dir_i, colors=['b'], labels=['scale'])
        ax.legend()
        fig.tight_layout()
        fig.savefig(plot_dir_i+'/accumulate_scale'+'_'+plot_traj.align_str+FORMAT,
                    bbox_inches='tight')
        print(Fore.GREEN +
